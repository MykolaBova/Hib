...
//		}
    ...
    static void testRemoveReference() {
    	...
    	// test removed reference is recreated
    	ses.begin();
        ...
        TestUte.assertTrue( ! x1.isNotDestroyed()); // x1 is destroyed
        x1 = y2.findReference(RefYY.XXRA);
        TestUte.assertTrue(x1.isNotDestroyed());
        TestUte.assertEqual("First2", x1.getString(RefXX.XNAME));
        ses.commit();
        
    	// test removed reference still works with savepoint
    	ses.begin(new SDataSet()); //, true);
        ...
        // test removed reference outisde a connection
        ...
        try {
        	x1 = y2.findReference(RefYY.XXRA);
        	TestUte.assertTrue(false); // Should fail here. x1 should have been removed from the dataSet 
        }
